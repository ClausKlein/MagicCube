...
class Cube():
    ...
    dictface = dict([(v, k) for k, v in list(facedict.items())])
    normals = [
    ...
    def move(self, f, l, d):
        ...
        if f == "U":
            ...
            for d in ds:
                self._rotate(
                    [
                        (self.facedict["F"], list(range(self.N)), l2),
                        (self.facedict["R"], list(range(self.N)), l2),
                        (self.facedict["B"], list(range(self.N)), l2),
                        (self.facedict["L"], list(range(self.N)), l2),
                    ]
                )
        if f == "D":
        ...
        if f == "F":
            ...
            for d in ds:
                self._rotate(
                    [
                        (self.facedict["U"], list(range(self.N)), l),
                        (self.facedict["L"], l2, list(range(self.N))),
                        (self.facedict["D"], list(range(self.N))[::-1], l2),
                        (self.facedict["R"], l, list(range(self.N))[::-1]),
                    ]
                )
        if f == "B":
        ...
        if f == "R":
            ...
            for d in ds:
                self._rotate(
                    [
                        (self.facedict["U"], l2, list(range(self.N))),
                        (self.facedict["F"], l2, list(range(self.N))),
                        (self.facedict["D"], l2, list(range(self.N))),
                        (self.facedict["B"], l, list(range(self.N))[::-1]),
                    ]
                )
        if f == "L":
    ...
    def _render_points(self, points, viewpoint):
        ...
        xdir = np.cross(np.array([0.0, 1.0, 0.0]), zdir)
    ...
    def render_views(self, ax):
        ...
        for viewpoint, shift in [
            (np.array([-x1, -x1, x2]), np.array([-1.5, 3.0])),
            (np.array([x1, x1, x2]), np.array([0.5, 3.0])),
            (np.array([x2, x1, -x1]), np.array([2.5, 3.0])),
        ]:
            for f, i in list(self.facedict.items()):
                zdir = self.normals[i]
    ...
    def _stickerpolygon(self, xdir, ydir, zdir, csz, j, k):
        ...
        return [
            ...
            zdir
            - xdir
            + (j + small + small) * csz * xdir
            - ydir
            + (k + small) * csz * ydir,
            zdir
            - xdir
            + (j + large - small) * csz * xdir
            - ydir
            + (k + small) * csz * ydir,
            zdir
            - xdir
