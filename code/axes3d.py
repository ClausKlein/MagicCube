...
class PolyView3D(plt.Axes):
    def __init__(self, view=(0, 0, 10), fig=None, rect=[0, 0, 1, 1], **kwargs):
        if fig is None:
            fig = plt.gcf()
    ...
    def poly3D_batch(self, xyzs, **kwargs):
        ...
        **kwargs :
        ...
        """
        N = len(xyzs)
        kwds = [dict([(key, kwargs[key][i]) for key in kwargs]) for i in range(N)]
        polys = [plt.Polygon(xyz[:, :2], **kwd) for (xyz, kwd) in zip(xyzs, kwds)]
