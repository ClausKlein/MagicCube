...
class CubeAxes(Axes):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.figure.text(
            ...
            0.05,
            (
                "Drag Mouse or use arrow keys to change "
                "perspective.\n"
    ...
    def draw_cube(self, rot=None, zloc=None):
        ...
        if self._cube_poly is None:
            self._cube_poly = [
                plt.Polygon(
                    self.faces[i, :, :2], facecolor=self.stickercolors[i], alpha=0.9
                )
                for i in range(6)
            ]
