...
class Cube:
    ...
    stickerwidth = 0.9
    stickermargin = 0.5 * (1.0 - stickerwidth)
    ...
    def _draw_cube(self):
        ...
        if self._face_polys is None:
            ...
            for i in range(len(colors)):
                fp = plt.Polygon(
                    faces[i], facecolor=plastic_color, zorder=face_zorders[i]
                )
                sp = plt.Polygon(
                    stickers[i], facecolor=colors[i], zorder=sticker_zorders[i]
                )
    ...
    def rotate_face(self, face, turns=1, layer=0, steps=5):
        if not np.allclose(turns, 0):
            for i in range(steps):
                self.cube.rotate_face(face, turns * 1.0 / steps, layer=layer)
                self._draw_cube()
